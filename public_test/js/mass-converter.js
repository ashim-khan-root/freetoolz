(function () {
  ...
  var UNITS = {
    ...
    gr: 0.0000647989, dwt: 0.00155517, slug: 14.5939
  };

  var UNIT_LABELS = {
    mg: 'Milligram', g: 'Gram', kg: 'Kilogram', t: 'Tonne',
    lb: 'Pound', oz: 'Ounce', st: 'Stone',
    gr: 'Grain', dwt: 'Pennyweight', slug: 'Slug'
  };
  ...
  var HTML =
    ...
    '<div class="conv-field"><label>From</label><select id="conv-from">' + opts('kg') + '</select></div>' +
    '<div class="conv-field"><label>To</label><select id="conv-to">' + opts('lb') + '</select></div>' +
    '</div>' +
    '<div class="conv-result-box">' +
    '<div class="conv-result" id="conv-result">22.0462</div>' +
    '<div class="conv-detail" id="conv-detail">10 kilograms = 22.0462 pounds</div>' +
    '</div>' +
  ...
  function convert() {
    ...
    var kg = val * UNITS[from];
    var result = kg / UNITS[to];
    ...
    var html = '<table>';
    var common = ['mg', 'g', 'kg', 't', 'lb', 'oz', 'st'];
    for (var i = 0; i < common.length; i++) {
      if (common[i] === to) continue;
      html += '<tr><td>' + UNIT_LABELS[common[i]] + '</td><td>' + (kg / UNITS[common[i]]).toFixed(4) + '</td></tr>';
    }
    html += '</table>';
